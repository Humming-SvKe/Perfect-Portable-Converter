<# Perfect Portable Converter (PPC) - OFFLINE build (refactor args, ASCII-safe) #>
...
function Resolve-FFTools {
  ...
  if (-not $FFMPEG -or -not (Test-Path $FFMPEG)) { Write-Log "ERROR: binaries\ffmpeg.exe not found"; return $false }
  if (-not $FFPROBE -or -not (Test-Path $FFPROBE)) { Write-Log "WARN: binaries\ffprobe.exe not found (continuing)" }
  return $true
}
...
function Convert-Batch {
  ...
  foreach ($f in $files) {
    try {
      ...
      # Build video args as array
      $vArgs=@()
      if ($p.vcodec) { $vArgs += @('-c:v', "$($p.vcodec)") }
      if ($p.preset) { $vArgs += @('-preset', "$($p.preset)") }
      if ($null -ne $p.crf) { $vArgs += @('-crf', "$($p.crf)") }

      # Build audio args as array
      $aArgs=@()
      if ($p.acodec) { $aArgs += @('-c:a', "$($p.acodec)") }
      ...
      $vfArgs=@(); if ($filters.Count -gt 0) { $vfArgs = @('-vf', ([string]::Join(',', $filters))) }
